...
newCategoryForm.addEventListener('submit', (e) => {
    ...
    if (isCategoryEmpty) {
        return console.log('Ajouter une tâche svp');
    }
...
// EVENT: Prendre l'ID de la Categorie
...
// EVENT: Prendre couleur sélectionné des catégories
...
// EVENT: chargement de l'édition todo avec valeurs
let todoToEdit = null;
...
// EVENT: Modification des todo après édition
...
function render() {
    ...
    // Mise en place des current viewing category
    if (!selectedCategoryId || selectedCategoryId === 'null') {
        currentlyViewing.innerHTML = `Vous êtes actuellement sur <strong>Toutes Categories</strong>`;
    } else {
        const currentCategory = categories.find((category) => category._id === selectedCategoryId);
        currentlyViewing.innerHTML = `Vous êtes actuellement sur <strong>${currentCategory.category}</strong> <span>(Supprimer)</span>`;
    }
}

function renderCategories() {
    categoriesContainer.innerHTML += `<li class="sidebar-item ${selectedCategoryId === 'null' || selectedCategoryId === null ? 'active' : ''}" data-category-id="">Voir tous</li>
	`;
...
function renderFormOptions() {
    ...
    newTodoSelect.innerHTML += `<option value="">Catégories</option>`;
    editTodoSelect.innerHTML += `<option value="">Catégories</option>`;
